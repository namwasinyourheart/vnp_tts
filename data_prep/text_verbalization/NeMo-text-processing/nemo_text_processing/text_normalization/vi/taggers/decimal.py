...
class DecimalFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool = True):
        ...
        fractional_part_single_digit = (
            pynutil.insert("fractional_part: \"")
            + single_digit_map
            + pynutil.insert("\"")
        )
        fractional_part_two_digit = (
            pynutil.insert("fractional_part: \"")
            + (pynini.closure(NEMO_DIGIT, 2, 2) @ cardinal_graph)
            + pynutil.insert("\"")
        )
        fractional_part_multi_digit = (
            pynutil.insert("fractional_part: \"")
            + single_digit_map
            + pynini.closure(pynutil.insert(" ") + single_digit_map, 2)
            + pynutil.insert("\"")
        )
        sep_comma = pynutil.insert(' separator: ","')
        sep_dot = pynutil.insert(' separator: "."')
        optional_quantity = (
        ...
        basic_decimal_two_digit = pynutil.add_weight(
            (integer_part + pynutil.insert(NEMO_SPACE)).ques
            + pynutil.delete(NEMO_COMMA)
            + sep_comma
            + pynutil.insert(NEMO_SPACE)
        ...
        basic_decimal_multi_digit = pynutil.add_weight(
            (integer_part + pynutil.insert(NEMO_SPACE)).ques
            + pynutil.delete(NEMO_COMMA)
            + sep_comma
            + pynutil.insert(NEMO_SPACE)
        ...
        patterns.append(basic_decimal_multi_digit)
        patterns.append(basic_decimal_multi_digit + optional_quantity)
        patterns.append(basic_decimal_two_digit)
        patterns.append(basic_decimal_two_digit + optional_quantity)
        patterns.append(basic_decimal)
        patterns.append(basic_decimal + optional_quantity)

        # Dot-decimal patterns: 0.73, 3.14
        dot_decimal_two_digit = pynutil.add_weight(
            (integer_part + pynutil.insert(NEMO_SPACE)).ques
            + pynutil.delete(".")
            + sep_dot
            + pynutil.insert(NEMO_SPACE)
        ...
        dot_decimal_multi_digit = pynutil.add_weight(
            (integer_part + pynutil.insert(NEMO_SPACE)).ques
            + pynutil.delete(".")
            + sep_dot
            + pynutil.insert(NEMO_SPACE)
        ...
        patterns.append(dot_decimal_multi_digit)
        patterns.append(dot_decimal_multi_digit + optional_quantity)
        patterns.append(dot_decimal_two_digit)
        patterns.append(dot_decimal_two_digit + optional_quantity)
        patterns.append(dot_decimal_single_digit)
        patterns.append(dot_decimal_single_digit + optional_quantity)
        ...
        separated_decimal_two_digit = pynutil.add_weight(
            separated_integer_part
            + pynutil.insert(NEMO_SPACE)
            + pynutil.delete(NEMO_COMMA)
            + sep_comma
        ...
        separated_multi_digit = pynutil.add_weight(
            separated_integer_part
            + pynutil.insert(NEMO_SPACE)
            + pynutil.delete(NEMO_COMMA)
            + sep_comma
        ...
        patterns.append(separated_multi_digit)
        patterns.append(separated_multi_digit + optional_quantity)
        patterns.append(separated_decimal_two_digit)
        patterns.append(separated_decimal_two_digit + optional_quantity)
