...
class MeasureFst(GraphFst):
    ...
    def __init__(
        ...
        graph_rate_denominator = pynini.string_map(
            [
                ("giờ", "giờ"),
                ("ngày", "ngày"),
                ("tuần", "tuần"),
        ...
        graph_unit_compound = (
            pynini.cross("/", " trên ") + pynutil.insert(NEMO_SPACE) + (graph_unit | graph_rate_denominator)
        )
        ...
        denominator_symbols = pynini.project(graph_rate_denominator, "input")
        simple_unit_pattern = metric_pattern | base_symbols | special_symbols

        # Add compound unit patterns to recognition
        compound_pattern = simple_unit_pattern + "/" + (simple_unit_pattern | denominator_symbols)
        ...
        grouped_integer = (
            ...
            + pynini.closure((pynutil.delete(".") | pynutil.delete(",")) + NEMO_DIGIT**3, 1)
        )
        integer_number = number | grouped_integer
        decimal_sep = pynini.union(NEMO_COMMA, pynini.accep("."))
        decimal_number = number + decimal_sep + pynini.closure(NEMO_DIGIT, 1, 2)
        ...
        integer_measure_domain = optional_negative_sign + integer_number + optional_space + unit_pattern
        ...
        cardinal_number_graph = pynutil.insert('integer: "') + (integer_number @ cardinal_graph) + pynutil.insert('"')
