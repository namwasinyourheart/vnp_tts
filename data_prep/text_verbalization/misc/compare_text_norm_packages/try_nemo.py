...
# "Công ty CP BOT Biên Cương sẽ cấp thẻ ưu tiên (tạm thời 3 tháng) để giảm giá dịch vụ sử dụng đường bộ đối với các chủ phương tiện có hộ khẩu ở TP Cẩm Phả.",
"Km153+450 và Km156+985 QL18",
...
"giao/nhận",
    ...
    "Tháng 10-2016, VKSND TP ban hành cáo trạng truy tố bà Hoa tội trộm cắp tài sản theo khoản 4 Điều 138 BLHS 1999 (hình phạt từ 12 năm đến tù chung thân)",
    # "Số liệu được công bố hồi cuối tuần trước cho thấy lạm phát Mỹ đạt 1,4% trong tháng 5 so với cách đây một năm, dưới mức mục tiêu của Fed là 2%.",
...
NEMO_ROOT = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../../NeMo-text-processing")
)
CACHE_DIR = os.path.abspath(
    os.path.join(os.path.dirname(__file__), ".nemo_cache")
)
...
AFTER_NEMO_IMPORT = perf_counter()


@lru_cache(maxsize=None)
def get_normalizer(lang: str = "vi") -> Normalizer:
    overwrite_cache = os.getenv("NEMO_OVERWRITE_CACHE", "false").lower() == "true"
    return Normalizer(input_case="cased", lang=lang, cache_dir=CACHE_DIR, overwrite_cache=overwrite_cache)
...
def nemo_normalize(text: str, lang: str = "vi") -> str:
    try:
...
def nemo_normalize_batch(texts, lang: str = "vi"):
    try:
        normalizer = get_normalizer(lang=lang)
        return normalizer.normalize_list(texts)
    except Exception:
        return [nemo_normalize(text, lang=lang) for text in texts]
...
def main():
    ...
    print("\n===== NeMo Normalization (Python API, Vietnamese) =====\n")
    print("Timing:")
    print(f"- Startup to NeMo import: {AFTER_NEMO_IMPORT - SCRIPT_START:.3f}s")
...
# rm -rf /home/nampv1/projects/tts/vnp_tts/data_prep/text_verbalization/compare_text_norm_packages/.nemo_cache
# python /home/nampv1/projects/tts/vnp_tts/data_prep/text_verbalization/compare_text_norm_packages/try_nemo.py