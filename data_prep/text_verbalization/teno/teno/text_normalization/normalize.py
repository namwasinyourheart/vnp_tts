...
try:
    from nemo_text_processing.text_normalization.vi.verbalizers.pre_processing import pre_process_text as _vi_pre_process_text
except Exception:
    _vi_pre_process_text = None
...
class Normalizer:
    ...
    def __init__(
        ...
        if lang != 'vi':
            raise NotImplementedError("Only Vietnamese (lang='vi') is supported in this build.")
    ...
    def normalize(
        ...
        punct_pre_process: bool = False,
        punct_post_process: bool = False,
        _token_level_fallback: bool = False,
    ) -> str:
        """
        ...
        debug_vi_punct = self.lang == "vi" and os.getenv("NEMO_VI_DEBUG_PUNCT", "false").lower() == "true"

        original_text = text
        if punct_pre_process:
            text = pre_process(text)
        if self.lang == "vi" and _vi_pre_process_text is not None:
            text = _vi_pre_process_text(text)
        text = text.strip()
        if not text:
            logger.debug(text)
            return text
        ...
        if debug_vi_punct:
            logger.info("VI_PUNCT_DEBUG normalize original=%r", original_text)
            logger.info("VI_PUNCT_DEBUG normalize pre_tag=%r", text)

        try:
            text_escaped = pynini.escape(text)
            tagged_lattice = self.find_tags(text_escaped)
            ...
            for s in split_tokens:
                ...
                if verbalizer_lattice is None or verbalizer_lattice.num_states() == 0:
                    raise ValueError(f"Empty verbalizer lattice for tokens: {s}")
                output += ' ' + Normalizer.select_verbalizer(verbalizer_lattice)

            output = SPACE_DUP.sub(' ', output[1:])

            if debug_vi_punct:
                logger.info("VI_PUNCT_DEBUG normalize pre_post_process=%r", output)

            if self.lang in ["en", "hi", "vi"] and hasattr(self, 'post_processor') and self.post_processor is not None:
                output = self.post_process(output)

            if debug_vi_punct:
                logger.info("VI_PUNCT_DEBUG normalize post_post_process=%r", output)
            ...
            if debug_vi_punct:
                logger.info("VI_PUNCT_DEBUG normalize post_punct_post_process=%r", output)
            return output
        except Exception as e:
            # Fail-soft: if a sentence cannot be normalized, try token-level normalization.
            if _token_level_fallback:
                logger.warning("Failed token: " + text + str(e))
                return text
    ...
    def post_process(self, normalized_text: 'pynini.FstLike') -> str:
        ...
        if self.post_processor is not None:
            if hasattr(self.post_processor, "process"):
                # Custom post-processor implementations typically expect plain text.
                normalized_text = self.post_processor.process(normalized_text)
            else:
