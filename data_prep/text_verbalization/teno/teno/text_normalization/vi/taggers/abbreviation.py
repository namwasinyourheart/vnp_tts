...
class AbbreviationFst(GraphFst):
    ...
    def __init__(self, deterministic: bool = True):
        ...
        letter_seq = upper_letter + pynini.closure(pynutil.insert(" ") + upper_letter, 1)
        slash_fallback_graph = letter_seq + pynini.closure(pynutil.delete("/") + pynutil.insert(", ") + letter_seq, 1)
        slash_fallback_graph = pynutil.add_weight(slash_fallback_graph, -0.01)
        fallback_graph = letter_seq
        fallback_graph = pynutil.add_weight(fallback_graph, 0.1)

        graph = (slash_fallback_graph | dict_graph | fallback_graph).optimize()
