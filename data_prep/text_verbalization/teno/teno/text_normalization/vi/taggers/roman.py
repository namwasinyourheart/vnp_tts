...
class RomanFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst, deterministic: bool = True):
        ...
        for i in range(1, 4000):
            ...
            valid_roman_pairs_upper.append((roman_upper, str(i)))

        roman_to_arabic_fst = pynini.string_map(valid_roman_pairs).optimize()
        roman_to_arabic_upper_fst = pynini.string_map(valid_roman_pairs_upper).optimize()

        roman_letter = pynini.union("I", "V", "X", "L", "C", "D", "M")
        roman_upper_min1 = pynini.closure(roman_letter, 1)
        roman_lower_letter = pynini.union("i", "v", "x", "l", "c", "d", "m")
        roman_lower_min2 = pynini.closure(roman_lower_letter, 2)
        roman_with_key = pynini.union(roman_upper_min1, roman_lower_min2)
        roman_to_arabic_with_key_fst = (roman_with_key @ roman_to_arabic_fst).optimize()
        roman_upper_min2 = pynini.closure(roman_letter, 2)
        roman_to_arabic_upper_min2_fst = (roman_upper_min2 @ roman_to_arabic_upper_fst).optimize()
        ...
        graph_with_key = (
            ...
            + pynini.compose(roman_to_arabic_with_key_fst, cardinal_graph)
            + pynutil.insert("\"")
        ).optimize()
        ...
        graph_without_key = (
            pynutil.add_weight(
                pynutil.insert("integer: \"")
                + pynini.compose(roman_to_arabic_upper_min2_fst, cardinal_graph)
                + pynutil.insert("\""),
                -2.0,
            )
        ).optimize()

        # Uppercase roman followed by punctuation without intervening space (e.g., "XIV;", "XIV.").
        punct = pynini.union(
            "!",
            "#",
            "%",
        ...
        self.fst = self.add_tokens(graph_with_key | graph_without_key | graph_without_key_with_punct).optimize()
